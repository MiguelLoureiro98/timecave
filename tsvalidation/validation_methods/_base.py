...
class base_splitter(ABC):
    ...
    def _ts_check(self, ts: np.ndarray | pd.Series) -> None:
        ...
        if(isinstance(ts, np.ndarray) is False and isinstance(ts, pd.Series) is False):
            ...
            raise ValueError("The time series must be univariate. Support for multivariate time series has yet to be implemented.\
    ...
    def _dim_check(self) -> None:
        ...
        if(self._n_splits > self._n_samples):
            ...
            raise ValueError("The number of splits must be smaller than (or equal to) the number of samples in the time series.");
