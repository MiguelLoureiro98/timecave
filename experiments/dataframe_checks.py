...
def check_time_column(df:pd.Dataframe, time_col_name='Date', freq: str or Timedelta or timedelta or DateOffset, fix:bool = False) -> pd.DataFrame:
    ...
    if fix:
        ...
        if nb_na_timesteps > 0:
            full_date_range_df = pd.DataFrame(index=pd.date_range(start=df_new[time_col_name].min(), end=df_new[time_col_name].max(), freq=freq))
            df_new = full_date_range_df.merge(df_new, left_index=True, right_on=time_col_name, how='left').reset_index(drop=True)
